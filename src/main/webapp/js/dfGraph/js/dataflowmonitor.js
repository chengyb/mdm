...
function createEditor(graphContainer){
	...
	editor.createTasks = function(div) {
		...
		if (this.graph != null)
		{
			mxUtils.para(div,  "流程定义:" + dataFlowName);
			//mxUtils.br(div);
			mxUtils.para(div,  "实例id:" + dataFlowInsId);
			//mxUtils.br(div);
			mxUtils.para(div,  "状态:" + state);
			//mxUtils.br(div);
			mxUtils.para(div,  "开始时间:" + startTime);
			//mxUtils.br(div);
			if (endTime != null && endTime.length > 0) {
				mxUtils.para(div,  "结束时间:" + endTime);
				//mxUtils.br(div);
			}
			if (state != 'SUSPENDED') {
				mxUtils.para(div,  "使用时间:" + usedTime);
				//mxUtils.br(div);
			}
		}
	};
...
function addAllTools(editor, graph, toolbar){
	//已经执行完的历史状态流程不能进行任何操作
	if (!isHistory) {
		_addTool(editor, graph, toolbar, mxBasePath+"/images/run.png", "run", "恢复流程");
		_addTool(editor, graph, toolbar, mxBasePath+"/images/stop.png", "stop", "挂起流程");
		_addTool(editor, graph, toolbar, mxBasePath+"/images/delete2.png", "remove" , "移除流程");
...
function addUserActions(editor, graph){
	//启动流程
	var run = function(editor, cell) {
		resumeDataFlow();
	};
	
	//挂起流程
	var stop = function(editor, cell) {
		suspendDataFlow();
	};
	
	//移除流程
	var remove = function(editor, cell) {
		removeDataFlow();
	};
	...
	editor.addAction("run", run);
	editor.addAction("stop", stop);
	editor.addAction("remove", remove);
...
function mouseEventMxGraph(graph){
	graph.addListener(mxEvent.DOUBLE_CLICK, function(sender, evt){
		...
		if(cell != null && cell.isEdge()){
			var obj = new Object();
			window.showModalDialog("/admin/df/transitionEdit?id=" + cell.id, obj, "dialogWidth=350px;dialogHeight=200px;resizable=yes;status=no;location=no;titlebar=no");
			evt.consume();
		}
	});
}
