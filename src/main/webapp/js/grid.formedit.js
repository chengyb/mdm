			...
			if($.fn.searchFilter) {
				...
				if( $("#"+fid).html() != null ) {
					...
					if(fields.length>0){
						...
						if (p.drag===true) {
							//edited by guoliang
							$(window.parent.document).find("#"+fid+" table thead tr:first td:first").css('cursor','move');
							//$("#"+fid+" table thead tr:first td:first").css('cursor','move');
							if(jQuery.fn.jqDrag) {
								//edited by guoliang
								$(window.parent.document).find("#"+fid).jqDrag($(window.parent.document).find("#"+fid+" table thead tr:first td:first"));
								//$("#"+fid).jqDrag($("#"+fid+" table thead tr:first td:first"));
							} else {
								try {
									//edited by guoliang
									$(window.parent.document).find("#"+fid).draggable({handle: $(window.parent.document).find("#"+fid+" table thead tr:first td:first")});
	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			function getFormData(){
				//edited by guoliang
				$(window.parent.document).find(".FormElement", "#"+frmtb).each(function(i) {
				//$(".FormElement", "#"+frmtb).each(function(i) {
					//edited by guoliang
					var celm = $(window.parent.document).find(".customelement", this);
			...
			function postIt() {
				...
				if(ret[0] === false) {
					//edited by guoliang
					$(window.parent.document).find("#FormError>td","#"+frmtb).html(ret[1]);
					//$("#FormError>td","#"+frmtb).html(ret[1]);
					//edited by guoliang
					$(window.parent.document).find("#FormError","#"+frmtb).show();
					//$("#FormError","#"+frmtb).show();
					// return; 
				}
			}
			...
			if ( $("#"+IDs.themodal).html() != null ) {
				...
				if(p.checkOnUpdate) {
					...
					$(window.parent.document).find("a.ui-jqdialog-titlebar-close","#"+IDs.themodal).unbind("click")
					.click(function(){
						if(!checkUpdates()) { return false; }
						$.jgrid.hideModal($(window.parent.document).find("#"+IDs.themodal,{gb:"#gbox_"+gID,jqm:p.jqModal,onClose: rp_ge.onClose}));
						return false;
					});
				}
				...
				//edited by guoliang
				//if(onInitializeForm) { onInitializeForm($(window.parent.document).find("#"+frmgr)); }
				if(onInitializeForm) { onInitializeForm($("#"+frmgr)); }
				//edited by guoliang
				//if(rowid=="_empty" || !rp_ge.viewPagerButtons) { $(window.parent.document).find("#pData,#nData","#"+frmtb+"_2").hide(); } else { $(window.parent.document).find("#pData,#nData","#"+frmtb+"_2").show(); }
				if(rowid=="_empty" || !rp_ge.viewPagerButtons) { $("#pData,#nData","#"+frmtb+"_2").hide(); } else { $("#pData,#nData","#"+frmtb+"_2").show(); }
				//edited by guoliang
				//if(onBeforeShow) { onBeforeShow($(window.parent.document).find("#"+frmgr)); }
				if(onBeforeShow) { onBeforeShow("#"+frmgr); }
				//edited by guoliang
				//$(window.parent.document).find("#"+IDs.themodal).data("onClose",rp_ge.onClose);
				$("#"+IDs.themodal).data("onClose",rp_ge.onClose);
				//edited by guoliang
				//$.jgrid.viewModal($(window.parent.document).find("#"+IDs.themodal,{gbox:"#gbox_"+gID,jqm:p.jqModal, overlay: p.overlay,modal:p.modal}));
				$.jgrid.viewModal("#"+IDs.themodal,{gbox:"#gbox_"+gID,jqm:p.jqModal, overlay: p.overlay,modal:p.modal});
				if(!closeovrl) {
					//edited by guoliang
					//$(window.parent.document).find(".jqmOverlay").click(function(){
					$(".jqmOverlay").click(function(){
						if(!checkUpdates()) { return false; }
						//edited by guoliang
						//$.jgrid.hideModal($(window.parent.document).find("#"+IDs.themodal,{gb:"#gbox_"+gID,jqm:p.jqModal, onClose: rp_ge.onClose}));
				...
				//edited by guoliang
				$(window.parent.document).find("#sData", "#"+frmtb+"_2").click(function(e){
				...
				//edited by guoliang
				$(window.parent.document).find("#cData", "#"+frmtb+"_2").click(function(e){
				...
				//edited by guoliang
				$(window.parent.document).find("#nData", "#"+frmtb+"_2").click(function(e){
				...
				//edited by guoliang
				$(window.parent.document).find("#pData", "#"+frmtb+"_2").click(function(e){
